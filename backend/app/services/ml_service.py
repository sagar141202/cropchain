...
def predict_yield(
    ...
    crop_area_interact = crop_enc * area_acres
    rain_area_interact = avg_rainfall * area_acres / 1000
    ...
    return {
        ...
        "confidence_min": round(max(predicted - confidence_range, 0), 2),
...
def detect_price_anomaly(
    ...
    if model is not None:
        ...
        score = round(abs(raw_score), 3)

    recommendations = {
        "Fair": "The offered price is fair. You can proceed with the sale.",
        "Slightly Low": f"Price is {round(deviation,1)}% below market. Negotiate for at least ₹{round(modal_price * 0.95)}/quintal.",
        "Exploitative": f"WARNING: Price is {round(deviation,1)}% below market rate. Contact your nearest APMC mandi or FPO immediately."
    }
