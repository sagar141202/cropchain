    ...
    <div className="min-h-screen page">
      ...
      <div className="max-w-sm mx-auto px-4 py-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-3xl p-8">
          ...
          {/* Role toggle */}
          <div className="flex gap-2 mb-6 p-1 rounded-2xl" style={{ background: "var(--glass2)", border: "1px solid var(--glass-border)" }}>
            {(["farmer","investor"] as const).map(r => (
              <button key={r} onClick={() => setRole(r)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all"
                style={role === r
                  ? { background: "white", color: "var(--text-1)", boxShadow: "var(--shadow)" }
