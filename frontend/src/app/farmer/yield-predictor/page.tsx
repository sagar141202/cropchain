  ...
  const predict = async () => {
    ...
    try {
      const res = await mlAPI.predictYield({
        ...
        season: form.season,
        state: form.state,
        avg_rainfall: Number(form.avg_rainfall),
        ...
        <div className="glass rounded-2xl p-5">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="field"><label className="field-label">Soil Type</label>
              <select value={form.soil_type} onChange={f("soil_type")}>
            ...
            <div className="field"><label className="field-label">Irrigation</label>
              <select value={form.irrigation_type} onChange={f("irrigation_type")}>
                {IRRIGATION_TYPES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
              </select></div>
            ...
            <div className="field"><label className="field-label">Rainfall (mm)</label>
              <input type="number" value={form.avg_rainfall} onChange={f("avg_rainfall")} /></div>
            <div className="field"><label className="field-label">Avg Temp (°C)</label>
              <input type="number" value={form.avg_temp} onChange={f("avg_temp")} /></div>
          </div>
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {[
                    ...
                    { label: "Min Estimate", value: `${result.confidence_min?.toFixed(1)} qtl`, icon: Thermometer, color: "#f59e0b", bg: "#fef3c7" },
                    { label: "Max Estimate", value: `${result.confidence_max?.toFixed(1)} qtl`, icon: Droplets, color: "#ec4899", bg: "#fce7f3" },
                  ].map(s => (
