...
import { LogOut, User, Mail, Phone, MapPin, Globe, Sprout } from "lucide-react";
    ...
    <div className="min-h-screen pb-24 page">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <motion.div initial={{ opacity:0, y:18 }} animate={{ opacity:1, y:0 }}
          ...
          <p className="text-xs mt-1" style={{ color:"var(--text-3)" }}>Farmer · CropChain Member</p>
        </motion.div>
          ...
          {fields.map(f => (
            <div key={f.label} className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                ...
                <f.icon className="w-4 h-4" style={{ color:"var(--green-dark)" }} />
              </div>
              <div>
        ...
        <div className="glass rounded-2xl p-4 mb-4 space-y-2">
          {[
            { href:"/farmer/yield-predictor", label:"Yield Predictor" },
            { href:"/farmer/proposals",       label:"My Proposals" },
            { href:"/farmer/coach",           label:"AI Coach" },
          ].map(l => (
